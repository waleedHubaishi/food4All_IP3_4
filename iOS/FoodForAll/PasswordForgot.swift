//
//  PasswordForgot.swift
//  login
//
//  Created by Jalil Hashemi on 30.11.16.
...
class PasswordForgot: UIViewController {
    ...
    @IBAction func verfiyOldEmailButton() {
        if isValidEmailAddress(emailAddressString: emailTF.text!) && (emailTF.text == email1 || emailTF.text == email2) {
            let secondViewController = self.storyboard?.instantiateViewController(withIdentifier: "PasswordForgot1") as! PasswordForgot1
